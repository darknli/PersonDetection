...
def detect_images(root, model, dst=None):
    ...
    for file in files:
        print(file)
        file = r"E:\Data\part_job\classroom_students_rec\bylw\test.png"
        ...
        cv2.waitKey()
        if dst is not None:
            save_root = os.path.join(root, dst)
            if not os.path.isdir(save_root):
                os.makedirs(save_root)
            print(f"保存{save_root}")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "-w", "--weight", type=str, default="saver/body_face_tiny/checkpoints/best.pth", help="model name"
    )
    ...
    dst = "111"
    detect_images(r"D:\temp_data\instance-level_human_parsing\Training\Images", model)